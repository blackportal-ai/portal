...
We want to leverage hardware acceleration on as many platforms as possible to
optimize machine learning workloads. While the current support is limited to
CPU, but macOS with **Metal** is shortly being worked on as the next step, the
framework is actively evolving to support a diverse range of computational
devices.
